	...
	%>	
		...
		<%if(request.getParameter("menu").equals("manage")){%>
			...
			<% if(productVO.getProTranCode().equals("1")){ %>
				<td align="left">판매완료<a href="/updateTranCodeByProd.do?prodNo=<%=productVO.getProdNo()%>&page=<%=searchVO.getPage()%>&ProtranCode=<%=productVO.getProTranCode()%>&menu=<%=request.getParameter("menu")%>">배송하기</a></td>
				
			<% }else if(productVO.getProTranCode().equals("2")){ %>
				<td align="left">배송중</td>
				
			<%} else if(productVO.getProTranCode().equals("0")) { %>
		...
		<%}else if((userVO == null || "user".equals(userVO.getRole()) && "search".equals(request.getParameter("menu")))) {%>
			<tr class="ct_list_pop">
			<td align="center"><%= i+1  %></td>
			<td></td>
			<%if(productVO.getProTranCode().equals("0")) {%>
				<td align="left"><a href="/getProduct.do?prodNo=<%=productVO.getProdNo()%>&menu=<%=request.getParameter("menu")%>"><%=productVO.getProdName() %></a></td>
			<%}else {%>
		...
		<%} else if("admin".equals(userVO.getRole()) && "search".equals(request.getParameter("menu"))) { %>
			<tr class="ct_list_pop">
			<td align="center"><%= i+1  %></td>
			<td></td>
			<td align="left"><a href="/getProduct.do?prodNo=<%=productVO.getProdNo()%>&menu=<%=request.getParameter("menu")%>"><%=productVO.getProdName() %></a></td>
